...
const page = () => {
  const signWithGoogle = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `https://sonvtixqerpgeuzhnvsw.supabase.co/auth/v1/callback`,
      },
    });
    if (error) {
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl shadow-blue-500/10 p-8 border border-white/20">
          ...
          <div className="space-y-4">
            <Button
              ...
              className="w-full h-12 bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 rounded-xl font-medium shadow-sm transition-all duration-200 hover:shadow-md group"
            >
